...
function MultipleLabelBlock(params) {
    // process parameters
    ...
    if (isArray(stimuliObj)) {
        ...
        //get the queue ready
       // stimuliObj.queue.installPlugin(createjs.Sound);
       // stimuliObj.queue.on("fileload", onPreloadFile, this);
       // stimuliObj.queue.on("progress", onLoadProgress, this);
      //  stimuliObj.queue.on("complete", onPreloadComplete, this);
      //  SCREW THIS PRELOADING... let's get one at a time.
      ...
      //  });
        ...
        for (op in opts) {
            ...
            case 'softInit':
                ...
                break;
            default:
                if (console) console.log('Unrecognized MultipleLabelBlock init option: ' +
                                         op + ' (' + opts[op] + ')');
            }
        }
...
//        $(this.auds).bind('ended.' + this.namespace, function() {_self.waitForResp();});
    ...
    next: function() {
        ...
        // create handler to turn on keyboard capture when stims end, and bind to stims
                         ...
                         //_self.auds[_self.stims[_self.n]].setAttribute('style', 'height: auto');
    ...
    info: function() {
        ...
        var curStimSrc = lastplayed;
        return [this.namespace + (this.practiceMode ? '.practice' : ''),
                this.n, this.stims[this.n], curStimSrc].join();
    },
...
////////////////////////////////////////////////////////////////////////////////
// CalibrationBlock extends MultipleLabelBlock, adding logistic regression fitting,
// error checking, etc.

function CalibrationBlock(params) {
    MultipleLabelBlock.call(this, params);
...
CalibrationBlock.prototype = {
    init: function(opts) {
        ...
        // call MultipleLabelBlock init() method via __super__ shortcut
    ...
    recordResp: function(e) {
        ...
        // call MultipleLabelBlock method
...
extend(CalibrationBlock, MultipleLabelBlock);
...
// link up via __super__ to superclass, etc.
extend(TestBlock, MultipleLabelBlock);
...
function updateChoices(c) {
    console.log("In update chocies");
    console.log(c);
    console.log(c[0]);
        $("#testInstructions").html('Press <span id="bKey" class="respKey"> X' + 
                                    '</span> for "' + c[0] + '"<br />' + 
                                    'Press <span id="dKey" class="respKey"> M' + 
                                    '</span> for "' + c[1] + '"');
}
